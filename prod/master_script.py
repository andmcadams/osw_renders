...
import create_all_renders
import rename_all_files
...
def main():
    ...
    rendering_valid = create_all_renders.validate_args(infile, cache, outdir, only_ids_file)
    renaming_valid = rename_all_files.validate_args(infile, outdir, only_ids_file, check_renders_dir=False)
    if not rendering_valid or not renaming_valid:
        exit(1)

    # Note that the renders_dir for renaming is the outdir for rendering
    create_all_renders.start_up(infile, cache, outdir, only_gender, only_render, only_ids_file)
    rename_all_files.start_up(infile, outdir, None, only_gender, only_render, only_ids_file)
